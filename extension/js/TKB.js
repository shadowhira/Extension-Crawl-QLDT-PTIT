    ...
    if (timetable) {
      ...
      prevButton.addEventListener("click", () => {
        if (currentWeekIndex > 0) {
          // Check to prevent going beyond the first week
          currentWeekIndex--;
      ...
      nextButton.addEventListener("click", () => {
        if (currentWeekIndex < weeks.length - 1) {
          // Check to prevent going beyond the last week
          currentWeekIndex++;
          weekSelector.selectedIndex = currentWeekIndex; // Update the selector
          displayTimetable(weeks[currentWeekIndex]);
        }
  ...
  function displayTimetable(week) {
    ...
    daysToDisplay.forEach((day) => {
      ...
      if (
        lessons &&
        lessons.length > 0 &&
        lessons[0] !== "Không có môn học nào."
      ) {
        ...
        lessons.forEach((lesson) => {
          ...
          html += `<td>${lesson.subject.split(" (")[0]}<br>${lesson.Phòng}<br>${
            lesson.time
          }</td>`;
        });
        html += "</tr>";
