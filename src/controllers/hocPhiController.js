const {runPuppeteer} = require('../services/hocPhiService'); // Đảm bảo bạn đã xuất hàm crawl từ file crawlXemHocPhi.js

// Controller để crawl dữ liệu học phí
const hocPhiController = {
  async crawlData(req, res) {
    try {
      const browser = await runPuppeteer(); // Chạy hàm crawlXemHocPhi
